...
namespace PontoWebIntegracaoExterna
{
    public partial class frmExemplo : Form
    {
        ...
        private void button28_Click(object sender, EventArgs e)
        {
            ...
            {
                ofd.Filter = "Arquivos CSV (*.csv)|*.csv|Arquivos de Texto (*.txt)|*.txt";
                ofd.Title = "Selecione o arquivo com a lista de funcionários";

                // 🔹 exibir diálogo para o usuário escolher
                ...
                try
                {
                    if (extensao == ".csv")
                    {
                        List<string[]> dadosCSV = LerCSV(caminhoArquivo);
                        MessageBox.Show("Arquivo CSV lido com sucesso!");
                    }
                    else if (extensao == ".txt")
                    {
                        List<string> linhasTXT = LerTXT(caminhoArquivo);
                        MessageBox.Show("Arquivo TXT lido com sucesso!");
                    }
                    else
                    {
                    ...
                    // 🔹 pedir pasta das fotos
                    using (FolderBrowserDialog fbd = new FolderBrowserDialog())
                    {
                        fbd.Description = "Selecione a pasta que contém as fotos dos funcionários";
                        if (fbd.ShowDialog() != DialogResult.OK)
                            return;

                        string pastaFotos = fbd.SelectedPath;
                        integracao.EnviarFuncionariosComFotos(caminhoArquivo, pastaFotos);
                        MessageBox.Show("Dados enviados com sucesso!");
                    }
                }
                catch (FileNotFoundException fnfEx)
                {
                    MessageBox.Show($"Erro: {fnfEx.Message}");
                }
                catch (Exception ex)
                {
        ...
        private List<string[]> LerCSV(string caminhoArquivo)
        {
            ...
            {
                string[] colunas = linhas[i].Split(';'); // Altere para ',' se necessário
                dados.Add(colunas);
            }
